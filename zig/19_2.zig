...
fn pointAffected(alloc: *Allocator, ints: []const Instr, x: Instr, y: Instr) !bool {
    var ints_cpy = try std.mem.dupe(alloc, Instr, ints);
    defer alloc.free(ints_cpy);

    var comp = IntcodeComputer.init(ints_cpy, alloc);
...
fn findLowerEdge(alloc: *Allocator, ints: []const Instr, x: Instr) !Instr {
    var in_beam = false;
    var y: Instr = 0;

    while (true) : (y += 1) {
        const affected = try pointAffected(alloc, ints, x, y);
        if (in_beam) {
            if (!affected) break;
        } else {
            if (affected) in_beam = true;
        }
    }
...
fn squareFits(alloc: *Allocator, ints: []const Instr, x: Instr) !bool {
    const lower_edge = try findLowerEdge(alloc, ints, x);
    if (lower_edge >= 100) {
        if (try pointAffected(alloc, ints, x + 99, lower_edge - 99)) {
            return true;
        }
    }
    return false;
